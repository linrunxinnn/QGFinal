...
//         <div class="icon">
//           <svg t="1744895638965" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="2522" width="20" height="20">
...
//             <svg t="1744897910864" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="6703" width="23" height="23">
...
//           </div>
//           <div class="item over">
//             <svg t="1744897970047" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="8118" width="20" height="20">
...
//             <svg t="1744897816015" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="3786" width="20" height="20">
...
//       const dropdown = e.target.closest(".item").querySelector(".drop-down");
...
//   // 绑定事件：移动任务状态
//   document.querySelectorAll(".drop-down .item.doing").forEach((item) => {
//     item.addEventListener("click", async (e) => {
//       const taskId = e.target.closest(".item[data-task-id]").dataset.taskId;
//       await updateTaskStatus(taskId, "doing");
//     });
//   });
//   document.querySelectorAll(".drop-down .item.over").forEach((item) => {
//     item.addEventListener("click", async (e) => {
//       const taskId = e.target.closest(".item[data-task-id]").dataset.taskId;
...
//   // 绑定事件：删除任务
//   document.querySelectorAll(".drop-down .item.remove").forEach((item) => {
//     item.addEventListener("click", async (e) => {
//       const taskId = e.target.closest(".item[data-task-id]").dataset.taskId;
  ...
  if (isAdmin) {
    taskSection
      ...
      .forEach((addIcon) => {
        addIcon.addEventListener("click", (e) => {
          ...
          addProgramSection.style.display = "block";
        });
      });
    ...
    addTaskHandler = debounce(async () => {
      ...
      if (!title || !groupId || !assignedTo) {
        alert("任务名称、小组和负责人是必填的");
        return;
      }

      try {
        // 使用 Promise.all 确保顺序执行
...
async function deleteTask(taskId) {
  ...
  try {
    const response = await fetch(
      `http://localhost:3000/program/${taskId}?projectId=${projectId}`,
      {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
